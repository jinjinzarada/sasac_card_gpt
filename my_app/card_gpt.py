...
if "pre_memory" not in st.sesstion_state:
...
def conversation_with_memory(question):
    ...
    # 4. sessrion_state["messages"]에 저장할 용도로 full_response 반환
...
if question:
    ...
    st.sesstion_state["messages"].append({"role": "user", "content": question})
    with st.chat_message("user"):
        st.write(question)

# 4. ai 답변 생성 & 출력
if st.sesstion_state["messages"][-1]["role"] != "assistant": # messages리스트에 담긴 메시지가 ai가 아닌 경우
    with st.chat_message("assistant"):
        try:
            ai_response = conversation_with_memory(question)
            st.sessiont_state["messages"].append({"role":"assistant", "content":ai_response})
